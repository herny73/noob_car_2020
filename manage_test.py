...
Usage:
    manage.py (drive) [--model=<model>] [--model_1=<model_1>] [--js] [--type=(linear|categorical|rnn|imu|behavior|3d|localizer|latent)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--myconfig=<filename>]
...
def drive(cfg, model_path=None, model_path_1=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    V.add(th_filter, inputs=['user/throttle'], outputs=['user/throttle'])     #滤除油门小于的数据，关于倒车zpy
    ...
    if "coral" in model_type:
        ...
        #在自动驾驶模式下加入detect部分
    V.add(Detect(cfg),inputs=['cam/image_array'],outputs=['match'],run_condition='run_pilot')
    ...
    class MyPilot:
        ...
        def update(self):
            while True:
                self.angle, self.throttle = self.run(self.match,self.angle_0,self.angle_1,self.throttle_0,self.throttle_1)
                #print(self.angle)
    ...
    V.add(MyPilot(), 
    inputs=['match','pilot/angle_0', 'pilot/throttle_0',
    'pilot/angle_1', 'pilot/throttle_1'], 
...
if __name__ == '__main__':
    ...
    if args['drive']:
        ...
        drive(cfg, model_path=args['--model'],model_path_1=args['--model_1'],use_joystick=args['--js'],
