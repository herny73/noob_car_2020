...
def drive(cfg, model_path=None, model_path_1=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    class MyButton:
        ...
        def update(self):
            while(True):
                self.state_out = self.run(self.button_in)
        def run_threaded(self,taga,tagb,tagc,tagd):
            self.button_in = [taga,tagb,tagc,tagd]
            return self.stage_out
        
        
    V.add(MyButton(),inputs=['taga','tagb','tagc','tagd'],outputs=['my_state'],threaded=True)
    ...
    class MyPilot:
        ...
        def run_threaded(self,mode,a0,t0,a1,t1):
            self.angle_0 = a0
            self.angle_1 = a1
            self.throttle_0 = t0
            self.throttle_1 = t1
        ...
        def update(self):
            while True:
                self.angle, self.throttle = self.run(self.mode,self.angle_0,self.angle_1,self.throttle_0,self.throttle_1)
                #print(self.angle)

        def run(self,pilot_mode,angle_0,throttle_0,angle_1,throttle_1):
            if(pilot_mode==0):
    ...
    V.add(MyPilot(), inputs=['mypilot_mod','pilot/angle_0', 'pilot/throttle_0','pilot/angle_1', 'pilot/throttle_1'], outputs=['mypilot/angle','mypilot/throttle'],threaded=True)

    #Choose what inputs should change the car.
    class DriveMode:
        def run(self, mode,
                    user_angle, user_throttle,
                    pilot_angle, pilot_throttle,pilot_mode,
                    mypilot_angle, mypilot_throttle, run_mypilot,taga):
            #print("drivemode is running")
            #print(taga,"!!!!!!!!",pilot_mode)
            if mode == 'user':
                print(taga,"!!!!!!!!",pilot_mode)
                if not(run_mypilot):
                    #print("user_angle:",user_angle)
    ...
    V.add(DriveMode(),
          inputs=['user/mode', 'user/angle', 'user/throttle', 'pilot/angle', 'pilot/throttle','mypilot_mod',
                  'mypilot/angle', 'mypilot/throttle', 'run_mypilot','taga'],
