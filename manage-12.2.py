...
def drive(cfg, model_path=None, model_path_1=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    class MyButton:
        ...
        def upadte(self):
            while(True):
                self.state_out = self.run(button_in)
        def run_threaded(self,taga,tagb,tagc,tagd):
            button_in = [taga,tagb,tagc,tagd]
            return stage_out
        
        
        V.add(MyButton(),inputs=['taga','tagb','tagc','tagd'],outputs=['my_state'])
    ...
    V.add(MyPilot(), inputs=['mypilot_mod','pilot/angle_0', 'pilot/throttle_0','pilot/angle_1', 'pilot/throttle_1'], outputs=['mypilot/angle','mypilot/throttle'])
